...
public class Code03_MaxAnimalNumber {
    ...
    // 编写练习
    public static int maxAnimals3(int[] arr) {
        if(arr==null||arr.length==0){
            return 0;
        }
        // 循环并在每个位置时候求当前位置最大为几层并进行记录此时层数的最小质量和
        int[] ends = new int[arr.length+1];//记录叠层对应的最小质量和
        ends[0] = 0;
        int size = 1;//记录当前能叠的最大层数
